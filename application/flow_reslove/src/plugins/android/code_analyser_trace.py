...
class CodeTrace:
    ...
    def fn_perform_code_trace(self, a, d, dx, code_trace_template, links):
        ...
        self.output_chains = []
        logging.info('before code trace analyse current links: ')
        logging.info(self.current_links) 
        ...
        logging.info('code trace analyse satisfied: {}, current links: '.format(bool_satisfied))
        logging.debug(self.current_links) 
    ...
    def fn_trace_through_code(self, trace_from_string, trace_to_string):
        ...
        if ((trace_from_list == []) or (trace_to_list == [])):
            logging.error('Either TraceFrom or TraceTo evaluated to None.')
            return False
    ...
    def fn_trace_reverse(self, class_part, method_part, desc_part,
        ...
        # logging.info("starting_points after fn_get_calls_to_method for {} {} {}".format(class_part, method_part, desc_part))
        # logging.info(starting_points)
        ...
        # logging.info(class_part)
        # logging.info("method_part:")
        # logging.info(method_part)
        # logging.info("desc_part:")
        # logging.info(desc_part)
        # logging.info("trace_chain:")
        # logging.info(trace_chain)
        mathcing_methods = self.inst_analysis_utils.fn_get_methods(class_part,method_part,desc_part)
        for method in mathcing_methods:
            starting_points.add(method.get_method())
        # logging.info("starting_points after find_methods for {} {} {}".format(class_part, method_part, desc_part))
        # logging.info(starting_points)
        # for case start activity with .class
        ttt = self.inst_analysis_utils.fn_get_calls_to_class_n(class_part)
        # logging.info("fn_get_calls_to_class for {}".format(class_part))
        # logging.info(ttt)
        starting_points.update(ttt)
        # logging.info("=====")
        # for t in ttt:
    ...
    def fn_analyse_trace_point(self, class_part, method_part, desc_part,
        ...
        if len(trace_chain_as_list) > self.trace_length_max:
            # logging.info('reach to trace_length_max {}'.format(self.trace_length_max));
            return
