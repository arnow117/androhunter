...
class AnalysisUtils:
    ...
    def fn_get_calls_to_class_n(self, class_part):
        ...
        for class_obj in class_objs:
            for caller_class, refs in class_obj.get_xref_from().items():
                for ref_kind,ref_method,ref_offset in refs:
                    # m = self.fn_get_methods(".",ref_method,".")
                    # logging.info("called from {} {} {}".format(caller_class.name,ref_kind,ref_method))
                    # logging.info(ref_method)
                    calling_methods.add(ref_method)
        return list(calling_methods)
